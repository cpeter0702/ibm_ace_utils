...
public class GetDocumentByCompositionId_JavaCompute extends MbJavaComputeNode {
	...
	public void evaluate(MbMessageAssembly inAssembly) throws MbException {
		...
		try {
			...
			String inputMsg = inRoot.getFirstElementByPath("/JSON/Data/*").toString();
			
			RSAPrivateKey privateKey = cUtils.getPrivateKey(PRIVATEKEY_PEM_PATH);
	        byte[] encodeMsg = cUtils.encodeByPrivateKey(privateKey, inputMsg);
	        String signStr = cUtils.base64Byte2Str(encodeMsg);
			
	        MbElement outRoot = outMessage.getRootElement();
	        // Create the Broker Blob Parser element
	        outRoot.getLastChild().getLastChild().createElementAsLastChild(MbElement.TYPE_NAME_VALUE, "signature", signStr.getBytes());
